	...
	for (i = 0; i < nqrdata; i++) {
		if (!mark[i]) {
		 ...
		 parity alone are too ambiguous.*/
						if (qrdata[j].sa_size == sa_size && qrdata[j].sa_parity == sa_parity && sa[qrdata[j].sa_index] < 0) {
					...
					case QR_MODE_NUM:
						{
							if (sa_ctext - sa_ntext >= (size_t)entry->payload.data.len){
								memcpy(sa_text + sa_ntext, entry->payload.data.buf, entry->payload.data.len * sizeof(*sa_text));
			...
			It requires copying buffers around to handle correctly.*/
					case QR_MODE_BYTE:
						{
							...
							if (eci < 0) {
								...
								if (inleft >=3 && in[0] == (char)0xEF && in[1] == (char)0xBB && in[2] == (char)0xBF) {
									...
									err = (utf8_cd == (iconv_t)-1 || iconv(utf8_cd, &in, &inleft, &out, &outleft) == (size_t)-1);
									if (!err) {
				 ...
				 conversion to the end of the list.*/
										if (ei < 2 && enc_list[ei] == latin1_cd && !text_is_latin1((unsigned char *)in, inleft)) {
											int ej;
											for (ej = ei + 1; ej < 3; ej++) {
												enc_list[ej-1] = enc_list[ej];
											}
								...
								err = (eci_cd == (iconv_t)-1 || iconv(eci_cd, &in, &inleft, &out, &outleft) == (size_t)-1);
								if (!err) {
									sa_ntext = out-sa_text;
								}
							}
					...
					case QR_MODE_KANJI:
						{
							...
							out = sa_text + sa_ntext;
							outleft = sa_ctext - sa_ntext;
							err = (sjis_cd == (iconv_t)-1 || iconv(sjis_cd, &in, &inleft, &out, &outleft) == (size_t)-1);
							if (!err) {
								sa_ntext = out-sa_text;
							}
						}
					...
					case QR_MODE_ECI:
						{
							...
							if (cur_eci <= QR_ECI_ISO8859_16 && cur_eci != 14) {
								if (cur_eci != QR_ECI_GLI0 && cur_eci != QR_ECI_CP437) {
									...
									/*Note that CP437 requires an iconv compiled with --enable-extra-encodings, and thus may not be available.*/
									enc = "CP437";
								}
							} else if (cur_eci == QR_ECI_SJIS) {
								enc = "SJIS";
							} else {
								/*Don't know what this ECI code specifies, but not an encoding that we recognize.*/
								continue;
							}
							eci = cur_eci;
